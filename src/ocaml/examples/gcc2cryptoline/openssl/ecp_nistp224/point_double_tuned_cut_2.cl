...
proc main(
  uint64 x0, uint64 x1, uint64 x2, uint64 x3,
  uint64 y0, uint64 y1, uint64 y2, uint64 y3,
  uint64 z0, uint64 z1, uint64 z2, uint64 z3,
  uint64 x_in2_0, uint64 x_in2_8, uint64 x_in2_16, uint64 x_in2_24,
  uint64 y_in6_0, uint64 y_in6_8, uint64 y_in6_16, uint64 y_in6_24,
  uint64 z_in3_0, uint64 z_in3_8, uint64 z_in3_16, uint64 z_in3_24,
  uint64 delta_0, uint64 delta_8, uint64 delta_16, uint64 delta_24,
  uint64 gamma_0, uint64 gamma_8, uint64 gamma_16, uint64 gamma_24,
  uint64 beta_0, uint64 beta_8, uint64 beta_16, uint64 beta_24,
  uint64 alpha_0, uint64 alpha_8, uint64 alpha_16, uint64 alpha_24
) =
{
  and [
    x_in2_0 = x0, x_in2_8 = x1, x_in2_16 = x2, x_in2_24 = x3,
    y_in6_0 = y0, y_in6_8 = y1, y_in6_16 = y2, y_in6_24 = y3,
    z_in3_0 = z0, z_in3_8 = z1, z_in3_16 = z2, z_in3_24 = z3,

    (* delta = z^2 *)
    (limbs 56 [delta_0, delta_8, delta_16, delta_24])
    =
    ((limbs 56 [z0, z1, z2, z3]) ** 2)
    (mod (2**224 - 2**96 + 1))
    ,
    (* gamma = y^2 *)
    (limbs 56 [gamma_0, gamma_8, gamma_16, gamma_24])
    =
    ((limbs 56 [y0, y1, y2, y3]) ** 2)
    (mod (2**224 - 2**96 + 1))
    ,
    (* beta = x * gamma *)
    (limbs 56 [beta_0, beta_8, beta_16, beta_24])
    =
    ((limbs 56 [x0, x1, x2, x3]) * (limbs 56 [gamma_0, gamma_8, gamma_16, gamma_24]))
    (mod (2**224 - 2**96 + 1))
    ,
    (* alpha = 3 * (x - delta) * (x + delta) *)
    (limbs 56 [alpha_0, alpha_8, alpha_16, alpha_24])
    =
    (
      3 *
      ((limbs 56 [x0, x1, x2, x3]) - (limbs 56 [delta_0, delta_8, delta_16, delta_24])) *
      ((limbs 56 [x0, x1, x2, x3]) + (limbs 56 [delta_0, delta_8, delta_16, delta_24]))
    )
    (mod (2**224 - 2**96 + 1))
  ]
