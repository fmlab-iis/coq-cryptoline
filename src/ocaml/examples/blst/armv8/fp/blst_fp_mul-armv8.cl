...
cut and [
  eqmod (limbs 64 [0, 0, 0, 0, 0, x19, x20, x21, x22, x23, x24, x25])
             ((limbs 64 [a0, a1, a2, a3, a4, a5]) *
              (limbs 64 [b0, b1, b2, b3, b4]))
  ...
  eqmod m0 1 2,
  eqmod (1 + m0 * n) 0 (2**64),
  limbs 64 [x26, mulHx11x17] = a0 * b5,
  limbs 64 [x27, mulHx12x17] = a1 * b5,
  limbs 64 [x28, mulHx13x17] = a2 * b5,
  limbs 64 [x0, mulHx14x17] = a3 * b5,
  limbs 64 [x1, mulHx15x17] = a4 * b5,
  limbs 64 [x3, mulHx16x17] = a5 * b5,
  x25 = 0
] && and [
  m0 = 0xb9feffffffffaaab@64, m1 = 0x1eabfffeb153ffff@64,
  m2 = 0x6730d2a0f6b0f624@64, m3 = 0x64774b84f38512bf@64,
  ...
  limbs 64 [b0, b1, b2, b3, b4, b5] <u limbs 64 [m0, m1, m2, m3, m4, m5],
  add 1@64 (mul m0 n) = 0@64,
  limbs 64 [x26, mulHx11x17] = uext a0 64 * uext b5 64,
  limbs 64 [x27, mulHx12x17] = uext a1 64 * uext b5 64,
  limbs 64 [x28, mulHx13x17] = uext a2 64 * uext b5 64,
  limbs 64 [x0, mulHx14x17] = uext a3 64 * uext b5 64,
  limbs 64 [x1, mulHx15x17] = uext a4 64 * uext b5 64,
  limbs 64 [x3, mulHx16x17] = uext a5 64 * uext b5 64,
  x25 = 0@64
];
...
assert eqmod (limbs 64 [0, 0, 0, 0, 0, 0,
             ...
             (limbs 64 [m0, m1, m2, m3, m4, m5]) && true;
assume eqmod (limbs 64 [0, 0, 0, 0, 0, 0,
                        r0, r1, r2, r3, r4, r5, r6])
             ((limbs 64 [a0, a1, a2, a3, a4, a5]) *
              (limbs 64 [b0, b1, b2, b3, b4, b5]))
             (limbs 64 [m0, m1, m2, m3, m4, m5]) && true;
...
ghost carry_old@uint1 : carry_old = carry && carry_old = carry;

(* sbcs	xzr, x25, xzr                              #! PC = 0x274877986932 *)
sbcs carry dontcare x25 0@uint64 carry;

assert true && carry_old = carry;
assume carry_old = carry && true;
