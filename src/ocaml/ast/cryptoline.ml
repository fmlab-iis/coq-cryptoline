...
let rec simplify_eexp e =
  ...
  | Ebinop (op, e1, e2) ->
     ...
     (match op, e1, e2 with
      ...
      | Epow, _, Econst n when Z.equal n Z.zero -> Econst Z.one
      | Epow, e, Econst n when Z.equal n Z.one -> e
      | _ -> Ebinop (op, e1, e2))
  | _ -> e
...
let eeqmod e1 e2 ms = Eeqmod (e1, e2, ms)
...
let rec eq_ebexp e1 e2 =
  ...
  | Eeqmod (e1a, e1b, e1c), Eeqmod (e2a, e2b, e2c) -> eq_eexp e1a e2a && eq_eexp e1b e2b && List.length e1c = List.length e2c && List.for_all2 eq_eexp e1c e2c
...
let rec subst_ebexp pats e =
  ...
  | Eeqmod (e1, e2, ms) -> Eeqmod (subst_eexp pats e1, subst_eexp pats e2, List.map (subst_eexp pats) ms)
...
let rec visit_ebexp visitor e =
  ...
  | DoChildren | ChangeDoChildrenPost _ ->
     ...
     f (match e with
        ...
        | Eeqmod (e1, e2, ms) -> Eeqmod (visit_eexp visitor e1, visit_eexp visitor e2, List.map (visit_eexp visitor) ms)
