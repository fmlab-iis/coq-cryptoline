  ...
  let res =
    ...
    | Drat ->
       unix (!Options.Std.drat_trim_path ^ " " ^ ifile ^ " "
             ...
             ^ " 2>& 1 > /dev/null")
    | Grat ->
       let gratl_file = tmpfile "coqcryptoline" ".gratl" in
       let gratp_file = tmpfile "coqcryptoline" ".gratp" in
       ...
       res
    | Lrat ->
       let lrat_file = tmpfile "coqcryptoline" ".lrat" in
       let tmp_file = tmpfile "coqcryptoline" ".tmp" in
...
let coq_cnf_unsat (id, cnf) =
  ...
  let ifile = tmpfile "coqcryptoline" ".cnf" in
