  ...
  let res =
    ...
    | Drat ->
       unix (!Options.Std.drat_trim_path ^ " " ^ ifile ^ " "
             ...
             ^ " 2>& 1 > /dev/null")
    | Grat ->
       let gratl_file = Filename.temp_file "coqcryptoline" ".gratl" in
       let gratp_file = Filename.temp_file "coqcryptoline" ".gratp" in
       ...
       res
    | Lrat ->
       let lrat_file = Filename.temp_file "coqcryptoline" ".lrat" in
       let tmp_file = Filename.temp_file "coqcryptoline" ".tmp" in
